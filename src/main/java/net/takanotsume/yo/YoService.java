...
public class YoService {
     ...
     * @throws YoFailedException
     */
    public static void sendYo(String token, String username)
            throws YoFailedException {
        
        List<NameValuePair> param = Form.form().add(PARAM_KEY_API_TOKEN, token)
                .add(PARAM_KEY_USERNAME, username).build();
        doPost(URL_YO, param);
    }
    
    private static void doPost(String url, List<NameValuePair> param)
            throws YoFailedException {
        ...
        try {
            ...
            response = Request.Post(url).bodyForm(param)
                    .addHeader("Content-Type", HEADER_CONTENT_TYPE).execute();
            
            HttpResponse httpResponse = response.returnResponse();
            int statusCode = httpResponse.getStatusLine().getStatusCode();
            
            if (statusCode != 201) {
                ((ByteArrayEntity) httpResponse.getEntity())
                        .writeTo(outputStream);
                throw new YoFailedException(
                        "Sending Yo was Failed. statusCode : " + statusCode
                                + " message : " + outputStream.toString());
            }
            
        } catch (IOException e) {
            throw new YoFailedException(
                    "Sending Yo was Failed. Exception thrown : "
                            + e.getMessage(), e);
        } finally {
            if (response != null) {
